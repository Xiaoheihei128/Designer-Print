 ...
 *
 * ★ PR-D:用户调尺寸字段 —— 二维码 改用「倍率」(scaleFactor),基准 1× = 30mm。
 *   regenerate 用 effectiveQrSizeMm() 把 display.scaleFactor → 实际 mm,
 *   QRCode.toCanvas 按目标 mm 渲染,canvas 几何精确跟随 display。
 */
import { FabricImage } from 'fabric'
import QRCode from 'qrcode'
import type { QrcodeControl, Segment, SegmentDisplayOpts } from '@op/types/control'
import { effectiveQrSizeMm } from '@op/core/layout-engine/qr-scale'
...
export class PrintQrcode extends FabricImage implements IPrintObject {
  ...
  async regenerate(): Promise<void> {
    ...
    try {
      // 2x 渲染保证缩放清晰(scaleFactor > 2 时 canvas 已 > 60mm,实际栅格更精细)
      await QRCode.toCanvas(canvas, text, {
        width: Math.ceil(targetPx * 2),
        margin: 1,
      ...
      const s = Math.min(targetPx / natW, targetPx / natH)
